...
#image_list = [cwd + s for s in image_names if s.lower().endswith(('label.png'))]

base_dir = Path(args.dir)
image_list = sorted(base_dir.walkfiles('*.label.png'))
#image_list = sorted(base_dir.walkfiles('*.png'))
#print(image_list[0])
...
def cal_class_weights(image_list):
    ...
    for k, (v1, v2) in count_all_pixels(image_list).items():
        ...
        freq_images[k] = v1 / (v2 * 480 * 640 * 1.0)        
...
with open('class_weight.txt', 'w') as f:
    for k, v in results.items():
        f.write("    class_weighting: %.4f\n" % round(v, 4))
